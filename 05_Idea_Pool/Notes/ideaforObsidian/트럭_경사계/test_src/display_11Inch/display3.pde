/**
 * Dump Truck Inclinometer (Two-Panel UI, Split Warnings) for 11-inch Display
 *
 * Left panel : SIDE view (Pitch)
 * Right panel: REAR view (Roll)
 *
 * Key change vs display2.pde
 * - WARNING/STATE is calculated separately for ROLL and PITCH (no combined max state).
 *
 * Notes
...
void setup()
{
    ...
    // Load images
    imgSide = loadImageRobust(SIDE_IMAGE_REL, SIDE_IMAGE_ABS, "SIDE");
    imgRear = loadImageRobust(REAR_IMAGE_REL, REAR_IMAGE_ABS, "REAR");
}

PImage loadImageRobust(String relPath, String absFallback, String tag)
{
    ...
    println("[IMG-" + tag + "] try rel: " + relPath);
    img = loadImage(relPath);

    if (img == null)
    {
        String p = sketchPath(relPath);
        println("[IMG-" + tag + "] try sketchPath: " + p);
        img = loadImage(p);
    }

    if (img == null && absFallback != null && absFallback.length() > 0)
    {
        println("[IMG-" + tag + "] try abs: " + absFallback);
        img = loadImage(absFallback);
    }

    if (img == null)
    {
        println("[IMG-" + tag + "] FAILED");
    }
    else
    {
        println("[IMG-" + tag + "] loaded: " + img.width + "x" + img.height);
    }
...
void draw()
{
    ...
    float rollAbs = abs(roll);
    float pitchAbs = abs(pitch);

    color rollStateColor = stateColorFor(rollAbs);
    color pitchStateColor = stateColorFor(pitchAbs);

    String rollStateText = stateTextFor(rollAbs);
    String pitchStateText = stateTextFor(pitchAbs);

    drawTopBarSplit(pitchStateText, pitchStateColor, rollStateText, rollStateColor);
    ...
    drawPanel(leftX, panelY, panelW, panelH, "SIDE (PITCH)", pitch, "PITCH", pitchStateText, pitchStateColor, true);
    drawPanel(rightX, panelY, panelW, panelH, "REAR (ROLL)", roll, "ROLL", rollStateText, rollStateColor, false);

    drawBottomHint();
}

color stateColorFor(float angleAbs)
{
    if (angleAbs > 25)
        return clrEmerg;
    if (angleAbs > 15)
...
String stateTextFor(float angleAbs)
{
    if (angleAbs > 25)
        return "EMERGENCY";
    if (angleAbs > 15)
...
void drawTopBarSplit(String pitchStateText, color pitchColor, String rollStateText, color rollColor)
{
    // Split bar: left for PITCH, right for ROLL
    noStroke();
    fill(pitchColor);
    rect(0, 0, width / 2, TOP_BAR_H);

    fill(rollColor);
    rect(width / 2, 0, width / 2, TOP_BAR_H);
    ...
    text("PITCH  " + pitchStateText, width * 0.25, TOP_BAR_H * 0.45);
    text("ROLL  " + rollStateText, width * 0.75, TOP_BAR_H * 0.45);

    // Small telemetry values
    textFont(fontSmall);
    text("PITCH " + formatDeg(pitch), width * 0.25, TOP_BAR_H - 18);
    text("ROLL  " + formatDeg(roll), width * 0.75, TOP_BAR_H - 18);
...
void drawPanel(int x, int y, int w, int h, String title, float angleDeg, String axisLabel, String stateText, color accent, boolean isSideView)
{
    // Panel background
    noStroke();
    fill(clrPanel);
    ...
    // Panel-local emergency flash (only for that axis)
    if (abs(angleDeg) > 25 && (frameCount / 14) % 2 == 0)
    {
        noStroke();
        fill(255, 0, 0, 35);
        rect(x, y, w, h, 14);
    ...
    noStroke();
    fill(accent);
    rect(x + 18, y + 18, 120, 28, 8);
    fill(0);
    textFont(fontSmall);
    textAlign(LEFT, CENTER);
    text(stateText, x + 28, y + 32);
    textAlign(CENTER, CENTER);
...
void drawTruckScene(int x, int y, int w, int h, float angleDeg, color accent, boolean isSideView)
{
    ...
    else
    {
        ...
        {
            imageMode(CENTER);
            image(imgRear, 0, 0, w * 0.85, h * 0.65);
        }
        else
        {
...
// Fallback vector drawings (used only if PNG fails to load)
