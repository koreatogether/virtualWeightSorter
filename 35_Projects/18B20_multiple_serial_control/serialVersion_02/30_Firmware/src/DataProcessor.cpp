...
void DataProcessor::printSensorTable()
{
    ...
    char buf[128];
    _comm->println((__FlashStringHelper *)TABLE_BORDER);
    _comm->println((__FlashStringHelper *)TABLE_HEADER);
    _comm->println((__FlashStringHelper *)TABLE_BORDER);
...
void DataProcessor::printInvalidSensorTable()
{
    ...
    char buf[64];
    _comm->println();
    _comm->println(F("--- Unassigned or Invalid ID Sensors Found ---"));
    _comm->println((__FlashStringHelper *)INVALID_TABLE_BORDER);
    _comm->println((__FlashStringHelper *)INVALID_TABLE_HEADER);
    _comm->println((__FlashStringHelper *)INVALID_TABLE_BORDER);
    ...
    _comm->println((__FlashStringHelper *)INVALID_TABLE_BORDER);
    _comm->println(F("Use Menu Option 2 -> 3 to auto-assign IDs."));
}
...
void DataProcessor::printAvailableIds()
{
    ...
    _comm->print((__FlashStringHelper *)USED_IDS_MSG);
    for (int i = 1; i <= SENSOR_COUNT; i++)
    {
        if (usedIds[i])
        {
            _comm->print(i);
            _comm->print((__FlashStringHelper *)SPACE_STR);
        }
    }
    _comm->println();
    _comm->print((__FlashStringHelper *)AVAILABLE_IDS_MSG);
    for (int i = 1; i <= SENSOR_COUNT; i++)
    {
        if (!usedIds[i])
        {
