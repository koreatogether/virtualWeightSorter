...
SerialUI ui(&comm, &dataProcessor, &sensorManager);

// App 클래스의 인스턴스를 생성하고, 위에서 만든 객체들을 주입합니다.
#if MEMORY_ANALYSIS_ENABLED
// 메모리 사용량 측정을 위한 인스턴스
MemoryUsageTester memTester;
App app(&sensorManager, &dataProcessor, &comm, &ui, &memTester);
#else
// 메모리 분석 없이 App 인스턴스 생성
App app(&sensorManager, &dataProcessor, &comm, &ui, nullptr);
#endif
...
void setup()
{
...
#if MEMORY_ANALYSIS_ENABLED
    Serial.println(F("=== DS18B20 + MEMORY ANALYSIS MODE ==="));

    // 초기 메모리 상태 출력
    if (memoryAnalysisEnabled)
    {
        Serial.println(F("=== INITIAL MEMORY STATUS ==="));
        memTester.printMemoryStatus("Before app initialization");
        memTester.printMemoryStatusCSV("setup", "start");
    }
#else
    Serial.println(F("=== DS18B20 SENSOR MODE ==="));
#endif
    ...
    if (memoryAnalysisEnabled)
    {
        ...
        Serial.println(F("=== INITIAL MEMORY ANALYSIS COMPLETE ==="));
    }
    Serial.println(F("System ready - DS18B20 sensors + Memory analysis"));
#else
    Serial.println(F("System ready - DS18B20 sensors"));
#endif
}
...
void loop()
{
    ...
    if (memoryAnalysisEnabled && (millis() - lastMemoryCheck > memoryCheckInterval))
    {
        ...
        Serial.println(F("=== PERIODIC MEMORY CHECK ==="));
    ...
    // App의 메인 루프 실행 (DS18B20 센서 동작 및 시리얼 명령어 처리 포함)
    app.run();
}
