...
### 7. 센서 가시성 및 진단 편의성 개선
- **문제**: 고유 ID(1~8)가 부여되지 않은 신규 센서(ID 0)가 메인 테이블에서 생략되어 사용자 혼란 가중.
- **해결**:
    - `SerialUI`가 메인 테이블 출력 후 미부여 센서 목록(`Unassigned sensors`)을 자동 노출하도록 개선.
    - 부팅 시 전체 센서 스캔 결과를 출력하고, ID 미설정 센서가 있을 경우 안내 메시지(⚠️) 표시.
    - 이를 통해 신규 센서 연결 시 즉시 확인이 가능하며 메뉴를 통해 바로 ID 부여 가능.

### 8. 최종 검증 결과
- **컴파일 성공**: PlatformIO 환경에서 경고 없이 빌드 완료.
- **업로드 및 동작**: XIAO ESP32-C3 (COM6) 업로드 완료 및 시리얼 통신 확인.
- **센서 인식**: GPIO 4(D2) 설정을 통해 4개의 센서가 정상적으로 인식됨을 확인.
