/*
https://wokwi.com/projects/400498388815259649
알려진 문제점 : 당첨번호 넣다가 잘못 넣은 숫자를 스페이스바로 지우고 백스페이스쓰고 다시 넣으면 0이 포함된 숫자로 안식됨
*/
...
void loop()
{
    ...
    // 사용자 복권 번호 입력 및 당첨 확인 (계속 실행)
    if (Serial.available() > 0 && bonusNumber != 0)
    { // 보너스 번호 입력 완료 후 실행
        String input = Serial.readStringUntil('\n');
        int numCount = 0;
        for (int i = 0; i < input.length(); i++)
