...
// 추가: LCD와 BLE 업데이트 간격 변수
unsigned long lastLCDUpdate = 0;
unsigned long lastBLEUpdate = 0;
unsigned long lastTempRequest = 0;
const unsigned long LCD_UPDATE_INTERVAL = 500;   // LCD 업데이트 간격 (500ms)
const unsigned long BLE_UPDATE_INTERVAL = 2000;  // BLE 업데이트 간격 (2초)
const unsigned long TEMP_REQUEST_INTERVAL = 500; // 온도 요청 간격 (500ms)
...
void updateLCDDisplay()
{
    ...
    // LCD 업데이트 간격 체크
    if (currentMillis - lastLCDUpdate < LCD_UPDATE_INTERVAL && !isFirstLCDRun)
    {
        return;
    }
...
void loop()
{
    ...
    // 온도 센서 요청 통합 (500ms 간격)
    if (currentMillis - lastTempRequest >= TEMP_REQUEST_INTERVAL)
    {
        ds18b20.requestTemperatures();
