/*
  ...
  https://create.arduino.cc/cloud/things/6fd596ca-09d1-4274-b27b-4e6eafd3fd1e
...
#include "thingProperties.h" // 아두이노 클라우드 프로퍼티 파일 포함
#include <Wire.h>
...
void setup()
{
    ...
    initProperties();                                  //* 아두이노 클라우드에 연결을 초기화
    ArduinoCloud.begin(ArduinoIoTPreferredConnection); //* 아두이노 IoT 클라우드에 연결
...
void loop()
{
    // 아두이노 IoT 클라우드와의 연결을 유지 및 관리
    ArduinoCloud.update();

    // 클라우드로 데이터 전송
    sendOrientationDataToCloud(); //* 10초마다 데이타를 전송하게 클라우드내설정됨

    if (millis() - previousMillis > repeatTime[0]) // 300ms마다 실행
    {
        previousMillis = millis();
        //* 당신이 원하는 코드를 넣으시면 됩니다.
    }
...
// 아두이노 클라우드에 pitch와 roll의 절대값을 보내는 함수 (인수 없이 전역 변수 사용)
void sendOrientationDataToCloud()
{
    // 아두이노 클라우드에 정의된 변수에 값을 할당
    // 예를 들어, cloudPitch와 cloudRoll이라는 변수가 있다고 가정
    cloudPitch = abs(pitch);
    cloudRoll = abs(roll);
    ...
    // // 디버깅을 위한 시리얼 출력
    // Serial.print("Pitch sent to cloud: ");
    // Serial.println(cloudPitch);
    // Serial.print("Roll sent to cloud: ");
    // Serial.println(cloudRoll);
}