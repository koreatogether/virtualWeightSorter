#!/usr/bin/env python3
"""Arduino 시뮬레이터 - ID 변경 테스트용"""

import threading
import time
import socket
import json
from datetime import datetime

class ArduinoSimulator:
    def __init__(self, port=8888):
        self.port = port
        self.running = False
        self.server_socket = None
        self.client_socket = None
        
        # 가상 센서 데이터
        self.sensors = {
            "285882840000000E": {"id": "01", "temp": 23.5},
            "28E6AA830000005A": {"id": "02", "temp": 24.2},
            "28FF123456789ABC": {"id": "03", "temp": 22.8},
        }
        
    def start(self):
        """시뮬레이터 시작"""
        self.running = True
        self.server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.server_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        
        try:
            self.server_socket.bind(('localhost', self.port))
            self.server_socket.listen(1)
            print(f"🤖 Arduino 시뮬레이터 시작 (포트: {self.port})")
            
            while self.running:
                try:
                    self.client_socket, addr = self.server_socket.accept()
                    print(f"📡 클라이언트 연결: {addr}")
                    
                    # 데이터 전송 스레드 시작
                    data_thread = threading.Thread(target=self.send_sensor_data)
                    data_thread.daemon = True
                    data_thread.start()
                    
                    # 명령 수신 처리
                    self.handle_commands()
                    
                except socket.error as e:
                    if self.running:
                        print(f"❌ 연결 오류: {e}")
                        time.sleep(1)
                        
        except Exception as e:
            print(f"❌ 시뮬레이터 오류: {e}")
        finally:
            self.cleanup()
    
    def send_sensor_data(self):
        """센서 데이터 주기적 전송"""
        while self.running and self.client_socket:
            try:
                for addr, data in self.sensors.items():
                    # 온도 약간 변동
                    data["temp"] += (time.time() % 10 - 5) * 0.1
                    
                    # Arduino 형식으로 데이터 전송
                    message = f"SENSOR_DATA:{data['id']}:{addr}:{data['temp']:.2f}\n"
                    self.client_socket.send(message.encode())
                    
                time.sleep(2)  # 2초마다 전송
                
            except socket.error:
                break
            except Exception as e:
                print(f"❌ 데이터 전송 오류: {e}")
                break
    
    def handle_commands(self):
        """Arduino 명령 처리"""
        buffer = ""
        
        while self.running and self.client_socket:
            try:
                data = self.client_socket.recv(1024).decode()
                if not data:
                    break
                    
                buffer += data
                
                # 완전한 명령 처리
                while '\n' in buffer:
                    line, buffer = buffer.split('\n', 1)
                    self.process_command(line.strip())
                    
            except socket.error:
                break
            except Exception as e:
                print(f"❌ 명령 처리 오류: {e}")
                break
    
    def process_command(self, command):
        """개별 명령 처리"""
        print(f"📨 수신된 명령: {command}")
        
        if command.startswith("SET_SENSOR_ID:"):
            self.handle_set_sensor_id(command)
        elif command == "GET_SENSORS":
            self.handle_get_sensors()
        else:
            print(f"❓ 알 수 없는 명령: {command}")
    
    def handle_set_sensor_id(self, command):
        """센서 ID 변경 명령 처리"""
        try:
            # SET_SENSOR_ID:주소:새ID 형식 파싱
            parts = command.split(":")
            if len(parts) != 3:
                self.send_response("ERROR:Invalid command format")
                return
                
            _, sensor_addr, new_id = parts
            
            # 센서 주소 확인
            if sensor_addr not in self.sensors:
                self.send_response(f"ERROR:Sensor not found: {sensor_addr}")
                return
            
            # ID 중복 체크
            for addr, data in self.sensors.items():
                if addr != sensor_addr and data["id"] == new_id:
                    self.send_response(f"ERROR:ID {new_id} already in use")
                    return
            
            # ID 변경 실행
            old_id = self.sensors[sensor_addr]["id"]
            self.sensors[sensor_addr]["id"] = new_id
            
            print(f"✅ ID 변경 성공: {sensor_addr} ({old_id} → {new_id})")
            self.send_response(f"SUCCESS:ID changed from {old_id} to {new_id}")
            
        except Exception as e:
            print(f"❌ ID 변경 오류: {e}")
            self.send_response(f"ERROR:Failed to change ID: {str(e)}")
    
    def handle_get_sensors(self):
        """센서 목록 요청 처리"""
        sensor_list = []
        for addr, data in self.sensors.items():
            sensor_list.append(f"{data['id']}:{addr}:{data['temp']:.2f}")
        
        response = "SENSORS:" + ",".join(sensor_list)
        self.send_response(response)
    
    def send_response(self, message):
        """응답 전송"""
        try:
            if self.client_socket:
                self.client_socket.send(f"{message}\n".encode())
                print(f"📤 응답 전송: {message}")
        except Exception as e:
            print(f"❌ 응답 전송 오류: {e}")
    
    def stop(self):
        """시뮬레이터 중지"""
        print("🛑 Arduino 시뮬레이터 중지 중...")
        self.running = False
        
        if self.client_socket:
            try:
                self.client_socket.close()
            except:
                pass
                
        if self.server_socket:
            try:
                self.server_socket.close()
            except:
                pass
    
    def cleanup(self):
        """리소스 정리"""
        self.stop()

def main():
    """시뮬레이터 실행"""
    simulator = ArduinoSimulator()
    
    try:
        print("🚀 Arduino 시뮬레이터 시작")
        print("📋 가상 센서 목록:")
        for addr, data in simulator.sensors.items():
            print(f"  - ID {data['id']}: {addr} ({data['temp']:.1f}°C)")
        print("\n💡 테스트 방법:")
        print("  1. 다른 터미널에서 대시보드 실행")
        print("  2. 'ID 변경' 버튼으로 모달 테스트")
        print("  3. Ctrl+C로 시뮬레이터 종료")
        print("-" * 50)
        
        simulator.start()
        
    except KeyboardInterrupt:
        print("\n⏹️  사용자가 중지했습니다")
    except Exception as e:
        print(f"❌ 시뮬레이터 오류: {e}")
    finally:
        simulator.cleanup()

if __name__ == "__main__":
    main()