...
#define PWMPin 9              // Pin connected to mosfet
...
void loop()
{
  ...
  unsigned long startCybleTime = millis();
  unsigned long lastSampleTime = 0;
  unsigned long sampleTime = 500; // Sampling frequency

  // 60s cycle
  oldTime = millis();
  while (millis() - startCybleTime <= (60 * 1000))
  {
  ...
  // 90s cycle
  startCybleTime = millis(); // reset cycle tracking for next period
  while (millis() - startCybleTime <= (90 * 1000))
  {
    if (millis() - lastSampleTime >= sampleTime)
    {
      // VL 1.4 Volts
      analogWrite(5, 20); // 35 is DC 1.4V output
      MQ7.update();       // Update data, the arduino will read the voltage from the analog pin
      MQ7.readSensor();   // Sensor will read PPM concentration using the model, a and b values set previously or from the setup
      MQ7.serialDebug();  // Will print the table on the serial port
      lastSampleTime = millis();
