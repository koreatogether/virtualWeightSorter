// ------------------------------------------------------------
// Advanced PMS7003 Serial Control Example (Arduino R4 WiFi) - Rev03
// ------------------------------------------------------------
...
//    4. Detailed Packet Formatting: (01) VAL || (02) VAL || ...
// ------------------------------------------------------------
...
// Global byte counter for packet indexing
uint8_t packetByteIndex = 0;
...
// Print a single byte in the selected format (Value Only)
void printFormattedValue(uint8_t b) {
...
// Helper to print indexed byte: "(01) FF || "
void printIndexedByte(uint8_t b, uint8_t idx) {
  Serial.print(F("("));
  if (idx < 10) Serial.print(F("0"));
  Serial.print(idx);
  Serial.print(F(") "));
  
  printFormattedValue(b);
...
int pmsRead() {
  ...
  if (c >= 0 && (monitor.mode == MON_RX || monitor.mode == MON_BOTH)) {
    ...
    // If new burst (timeout > 50ms), reset index and print newline
    ...
    if (monitor.mode != MON_OFF) {
      packetByteIndex++;
      printIndexedByte((uint8_t)c, packetByteIndex);
    }
  }
  return c;
}
...
void printMenu() {
  Serial.println(F("\n---------------------------------------------"));
  Serial.println(F(" PMS7003 Control Menu (Rev03)"));
  Serial.println(F("---------------------------------------------"));
...
void setup() {
  ...
  Serial.println(F("\n--- PMS7003 Advanced Control Rev03 ---"));
