/**
 * @file sleepMode_tftLCDgraphic_rev04.ino
 * @brief PMS7003 TFT LCD GUI with Touch Control (Screen Saver Added)
 *
 * Rev04 Changes:
 * - 화면 보호 기능 추가 (LCD 백라이트 OFF / 화면 끄기)
 * - 터치 시 화면 깨우기 기능
 * - 일정 시간 터치 없으면 화면 자동 OFF
 *
 * Rev03 Changes:
 ...
 *
 * Note: TFT LCD 백라이트 제어
 * - 쉴드형 LCD는 백라이트 핀이 없거나 항상 ON인 경우가 많음
 * - 이 경우 화면을 검은색으로 채워 LCD 보호 (의사 절전)
 * - 백라이트 핀이 있는 경우 해당 핀을 LOW로 설정
 */
...
namespace Config
{
    ...
    // 백라이트 핀 (쉴드에 따라 다름, 없으면 -1)
    // 일반적인 쉴드: A0, 3, 10 등 확인 필요
    // 백라이트 핀이 없는 쉴드는 -1로 설정 (화면 검정으로 대체)
    constexpr int8_t BACKLIGHT_PIN = -1; // -1 = 백라이트 핀 없음
    ...
    // rev04: 화면 보호 설정
    constexpr unsigned long SCREEN_OFF_TIMEOUT = 60000; // 60초 후 화면 끄기
}
...
struct SystemState
{
    ...
    // rev04: 화면 보호 상태
    unsigned long lastInteractionTime = 0; // 마지막 사용자 상호작용 시간
    bool isScreenOff = false;              // 화면 꺼짐 상태
...
// -----------------------------------------------------------------------------
// 4. Screen Control (rev04: 백라이트/화면 제어)
// -----------------------------------------------------------------------------
namespace Screen
{
    // 백라이트 켜기
    ...
    void backlightOff()
    {
        if (Config::BACKLIGHT_PIN >= 0)
        {
            digitalWrite(Config::BACKLIGHT_PIN, LOW);
        }
    }
    ...
    void turnOff()
    {
        if (!sys.isScreenOff)
        {
            sys.isScreenOff = true;

            // 백라이트 핀이 있으면 끄기
            backlightOff();

            // 화면을 검은색으로 채우기 (LCD 보호 + 전력 절약)
            tft.fillScreen(Color::BLACK);

            Serial.println(F(">>> 화면 보호 모드 진입 (Screen OFF)"));
        }
    }
    ...
    void turnOn()
    {
        if (sys.isScreenOff)
        {
            sys.isScreenOff = false;

            // 백라이트 핀이 있으면 켜기
            backlightOn();

            // 화면 다시 그리기
            prev.initialDrawDone = false; // 전체 화면 갱신 강제

            Serial.println(F(">>> 화면 보호 모드 해제 (Screen ON)"));
        }
    }
    ...
    void checkTimeout()
    {
        if (!sys.isScreenOff)
        {
            ...
            if (now - sys.lastInteractionTime >= Config::SCREEN_OFF_TIMEOUT)
            {
                turnOff();
            }
        }
...
namespace UI
{
    ...
    constexpr int SETTINGS_TITLE_Y = TAB_HEIGHT + 10;
    constexpr int SETTINGS_BTN_START_Y = TAB_HEIGHT + 55;
    constexpr int SETTINGS_BTN_HEIGHT = 35;
    constexpr int SETTINGS_BTN_GAP = 8;
...
// -----------------------------------------------------------------------------
// 8. Touch Event Handling (rev04: 화면 깨우기 추가)
// -----------------------------------------------------------------------------
bool handleTouch()
{
    unsigned long now = millis();
    ...
    // rev04: 사용자 상호작용 기록 (화면 보호 타이머 리셋)
    Screen::recordInteraction();

    // rev04: 화면이 꺼져 있었다면, 터치로 깨우기만 하고 다른 동작 안함
    if (sys.isScreenOff)
    {
        // 이미 recordInteraction()에서 화면을 켰음
        return true;
    }
...
void setup()
{
    ...
    Serial.println(F("PMS7003 TFT LCD GUI (Rev04) Starting..."));
    ...
    Serial.print(F("Screen OFF timeout: "));
    Serial.print(Config::SCREEN_OFF_TIMEOUT / 1000);
...
void loop()
{
    ...
    // 2. 시스템 상태 갱신 (화면 상태와 무관하게 센서는 계속 동작)
    updateSystem();

    // 3. rev04: 화면 보호 타이머 체크
    Screen::checkTimeout();

    // 4. 모니터 탭: 주기적 데이터 갱신 (화면이 켜져 있을 때만)
    if (!sys.isScreenOff && sys.currentTab == AppTab::MONITOR)
    {
        if (sys.isSensorSleeping &&
