...
CONFIG_PATH = os.path.join(CWD, "config_esp32c3.json")
...
def save_data():
    ...
    for target in targets:
        try:
            ...
            # ESP32-C3 version check for password if needed, for now using same logic as R4
            ...
            if response.status_code == 200:
                if not is_online:
                    print(f"\n[{get_color(Colors.GREEN)}LINKED{get_color(Colors.END)}] 장치(ESP32-C3 Xiao) 연결됨: {target}")
                    is_online = True
                ...
                with open(file_path, "w", encoding="utf-8") as f:
                    # ESP32-C3 may have different field names if modified, but current main.cpp uses s1, s2, s3, s4
    ...
    if is_online:
        ...
        print(f"\r[{curr_time}] ESP32-C3 Xiao 검색 중... ({device['hostname']} 또는 {device['ip_address']})", end="", flush=True)
...
if __name__ == "__main__":
    ...
    print(f" Boiler Data Collector for ESP32-C3 Xiao")
