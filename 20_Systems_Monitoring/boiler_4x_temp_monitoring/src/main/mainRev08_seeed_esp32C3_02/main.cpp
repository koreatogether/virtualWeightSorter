...
void setup()
{
    ...
    initNextion();
    Serial.println("[System] Nextion Manager Initialized.");

    initSensors();
    Serial.println("[System] Sensors Initialized.");

    // WiFi 연결 전에 현재 온도와 시계를 한 번 전송하여 화면이 나오게 함
    readTemperatures();
    updateNextionDisplay();
    Serial.println("[System] Initial Data Sent to Display.");

    initWiFi();
    Serial.println("[System] WiFi Setup Finished.");
