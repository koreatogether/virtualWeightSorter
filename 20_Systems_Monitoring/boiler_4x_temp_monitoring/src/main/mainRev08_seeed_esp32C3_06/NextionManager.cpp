...
/**
 * @brief Nextion 명령 전송 (종료 코드 0xFF 0xFF 0xFF 포함)
 */
...
bool isNextionConnected = false;

void initNextion()
{
    // 1. 9600bps로 초기화 (Nextion 기본 속도에서 시작)
    MySerial1.begin(9600, SERIAL_8N1, RX1_PIN, TX1_PIN);
#if DEBUG_ENABLE
    Serial.println(F("[Nextion] 보드레이트 변경을 위해 9600bps로 시작..."));
#endif
    ...
    // 3. 통신 속도 변경 명령 (임시 변경 baud=115200)
...
#if DEBUG_ENABLE
    Serial.println(F("[Nextion] 115200bps로 전환 완료"));
#endif

    // 5. 핸드셰이크: 응답을 기다리는 명령 전송 (bkcmd=3: 모든 명령에 대한 응답 받기)
    // 여기서는 간단히 페이지 번호를 요청하여 응답이 오는지 확인
    sendNextionCommand("sendme");
    ...
    while (millis() - start < 500)
    {
        ...
        {
            uint8_t res = HMISerial.read();
            if (res == 0x66) // 0x66: 페이지 번호 응답 시작 바이트
            {
                isNextionConnected = true;
                break;
            }
        }
    }

    if (isNextionConnected)
    {
        Serial.println(F("[Nextion] 핸드셰이크 성공: 연결됨"));
        sendNextionCommand("t0.txt=\"System: Online\"");
    }
    else
    {
        Serial.println(F("[Nextion] 핸드셰이크 실패: 응답 없음"));
    }
...
{
    // 메인 루프에서 새로운 데이터가 있을 때만 호출되므로 내부 타이머 제거
    updateTimeOnDisplay();

    int plotValues[4];
    for (int i = 0; i < 4; i++)
    {
        // 온도를 0~100도 범위에서 그래프 높이에 맞게 매핑
        ...
        // 숫자 값 업데이트 (x10 포맷으로 전송하여 소수점 첫째자리 표시)
        char cmd[32];
    ...
    // 그래프 선 연결을 위한 보간 처리 (LINE_STEPS 만큼 add 명령 전송)
    for (int step = 1; step <= LINE_STEPS; step++)
    {
        for (int ch = 0; ch < 4; ch++)
        {
        ...
        globalPointCount++;
        delay(SEND_DELAY_MS); // 시리얼 버퍼 오버플로우 방지
    }
