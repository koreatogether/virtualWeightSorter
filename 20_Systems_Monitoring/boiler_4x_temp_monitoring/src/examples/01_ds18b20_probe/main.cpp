...
/*
임의 온도센서 4개를 연결하면 우선 4개의 고정 주소중 뒷자리 4자리가 보이게 하는 장면 추가 할것
...
void updateSystem()
{
    ...
    if (now - lastSampleMs >= SAMPLE_INTERVAL_MS)
    {
        ...
        if (foundSensors == 0)
        {
            // 센서가 없는 경우 주기적으로 안내 메시지를 출력
            if (now - lastMissingSensorReminderMs >= MISSING_SENSOR_REMINDER_MS)
            {
                Serial.println("add sensor please");
                lastMissingSensorReminderMs = now;
            }
            return;
        }
