...
## [2026-02-07 15:20] v05 폴더 생성 및 EXE 패키징 기반 마련

### 진행 내용
- **v05 환경 구성**: `mainRev08_seeed_esp32C3_05` 폴더 생성 및 `platformio.ini`에 `xiao_esp32c3_rev08_05` 빌드 환경 추가.
- **EXE 패키징 테스트 성공**: `PyInstaller`를 사용하여 분석기 서버를 단일 `.exe` 파일로 빌드 완료.
- **경로 로직 최적화**: EXE 실행 시 내부 리소스(HTML)와 외부 로그 폴더(`docs/logs`)를 모두 정상적으로 참조할 수 있도록 `analyzer_server.py` 코드 수정.
