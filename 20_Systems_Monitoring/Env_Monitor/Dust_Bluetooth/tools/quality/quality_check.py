...
def generate_report(results: dict[str, tuple[bool, str]]) -> None:
    ...
    # Generate JSON report in logs folder with timestamp prefix
    log_dir = Path("tools/quality/logs")
    log_dir.mkdir(exist_ok=True)

    timestamp = datetime.now().strftime("%Y_%m_%d_%H_%M_%S")
    report_file = log_dir / f"{timestamp}_품질_검사_결과.json"
    ...
    # Also save to old reports folder for backward compatibility
    report_dir = Path("tools/quality/reports")
    report_dir.mkdir(exist_ok=True)
    old_timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    old_report_file = report_dir / f"quality_report_{old_timestamp}.json"
...
def main():
    ...
    print("Weigher-Sorter Project Code Quality Check Starting")
